/**
 * Seeds actors with disabilities — deaf, blind, mobility, neurodivergent, little people, chronic illness.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-disability-actors.ts
 */
...
const ACTORS = [
  ...
  'Marlee Matlin', 'Lauren Ridloff', 'Ali Stroker',

  // Cerebral palsy and mobility
  'RJ Mitte', 'Micah Fowler', 'Maysoon Zayid', 'Sophie Morgan',
  'Ruth Madeley', 'Cherylee Houston', 'Julie Fernandez',
  'Sophie Christiansen', 'Cerrie Burnell',

  // Little people
  'Peter Dinklage', 'Warwick Davis', 'Danny Woodburn',
  'Meredith Eaton', 'Linda Hunt',

  // Down syndrome
  'Jamie Brewer', 'Lauren Potter', 'Chris Burke',
  'Tommy Jessop', 'Sarah Gordy', 'Zack Gottsagen',

  // Autism spectrum
  'Kayla Cromer', 'Mickey Rowe',

  // Amputee and limb difference
  'Mat Fraser', 'Adam Pearson', 'Jillian Mercado',
  'Aaron Philip', 'Liz Carr', 'Rosie Jones', 'Sydelle Noel',

  // Chronic illness and neurological
  'Michael J Fox', 'Selma Blair', 'Montel Williams',
  'Teri Garr',

  // Comedy / entertainment with disabilities
  'Josh Blue', 'Lost Voice Guy',

  // Mainstream stars with disabilities or known advocacy
  'Emily Blunt', 'Samuel L Jackson', 'Joaquin Phoenix',
  'Anthony Hopkins', 'Dan Aykroyd', 'Paddy Considine',
  'Daryl Hannah', 'Shia LaBeouf', 'Cheri Oteri',

  // Additional
  'Kayla Martell', 'Shannon Murray',
]
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'drama; disability',
        casting_profile,
        notes: 'seeded via seed-disability-actors.ts',
      }, { onConflict: 'tmdb_id' })
