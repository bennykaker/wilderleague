/**
 * Seeds Ocean's Eleven/Twelve/Thirteen and Ocean's Eight cast members.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-oceans-actors.ts
 */
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'crime; heist; comedy; film',
        casting_profile,
        notes: 'seeded via seed-oceans-actors.ts',
      }, { onConflict: 'tmdb_id' })
