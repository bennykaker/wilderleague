/**
 * Seeds ER and House M.D. cast members.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-er-house-actors.ts
 */
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'drama; medical; television; procedural',
        casting_profile,
        notes: 'seeded via seed-er-house-actors.ts',
      }, { onConflict: 'tmdb_id' })
