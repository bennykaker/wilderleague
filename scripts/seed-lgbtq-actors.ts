/**
 * Seeds LGBTQ+ actors and performers.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-lgbtq-actors.ts
 */
...
const ACTORS = [
  // American — mainstream crossover
  'Neil Patrick Harris', 'Jim Parsons', 'Zachary Quinto', 'Matt Bomer',
  'Wentworth Miller', 'Colton Haynes', 'Luke Macfarlane', 'Jonathan Groff',
  'Wilson Cruz', 'Guillermo Diaz', 'T.R. Knight', 'Jesse Tyler Ferguson',

  // British — established
  'Alan Cumming', 'Rupert Everett', 'Ian McKellen', 'Derek Jacobi',
  'Stephen Fry', 'Mark Gatiss', 'Russell Tovey', 'Andrew Scott',
  'Ben Whishaw', 'Olly Alexander',

  // Trans and non-binary actors
  'Elliot Page', 'Laverne Cox', 'MJ Rodriguez', 'Indya Moore', 'Billy Porter',

  // American comedians / character actors
  'Niecy Nash', 'Samira Wiley', 'Lea DeLaria', 'Lena Waithe',
  'Sara Ramirez', 'Stephanie Beatriz', 'Tig Notaro', 'Wanda Sykes',
  'Jane Lynch', 'Kate McKinnon', 'Hannah Gadsby', 'Fortune Feimster',
  'Mae Martin',

  // British comedy / entertainment
  'Noel Fielding', 'Matt Lucas', 'David Walliams', 'Julian Clary',
  'Paul O\'Grady', 'John Inman',
]
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        casting_profile,
        notes: 'seeded via seed-lgbtq-actors.ts',
      }, { onConflict: 'tmdb_id' })
