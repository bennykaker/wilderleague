/**
 * Seeds Downton Abbey cast members — series regulars and recurring/guest players.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-downton-abbey-actors.ts
 */
...
const ACTORS = [
  ...
  // Distinguished guest and recurring
  'Maggie Smith',
  'Penelope Keith',
  'Geraldine James',
  'David Robb',
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'drama; television; period drama; british',
        casting_profile,
        notes: 'seeded via seed-downton-abbey-actors.ts',
      }, { onConflict: 'tmdb_id' })
