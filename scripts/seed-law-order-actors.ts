/**
 * Seeds Law & Order franchise cast — original, SVU, Criminal Intent, Trial by Jury, LA, OC, For the Defense.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-law-order-actors.ts
 */
...
const ACTORS = [
  // Law & Order original
  'Jerry Orbach', 'Chris Noth', 'Michael Moriarty', 'Sam Waterston',
  'Steven Hill', 'Dann Florek', 'Benjamin Bratt', 'Angie Harmon',
  'Jesse L. Martin', 'S. Epatha Merkerson', 'Fred Thompson',
  'Dianne Wiest', 'Elisabeth Rohm', 'Dennis Farina',
  'Anthony Anderson', 'Alana De La Garza', 'Linus Roache',
  'Jeremy Sisto', 'Jeff Goldblum', 'Annie Parisse',

  // SVU
  'Mariska Hargitay', 'Christopher Meloni', 'Richard Belzer',
  'Ice-T', 'Stephanie March', 'BD Wong', 'Tamara Tunie',
  'Adam Beach', 'Danny Pino', 'Kelli Giddish', 'Peter Scanavino',
  'Philip Winchester', 'Raul Esparza', 'Demore Barnes',
  'Mike Doyle', 'Jamie Gray Hyder', 'Octavio Pisano',
  'Molly Burnett', 'Ainsley Seiger',

  // Criminal Intent
  'Vincent D\'Onofrio', 'Kathryn Erbe', 'Jamey Sheridan',
  'Courtney B. Vance', 'Eric Bogosian', 'Mary Elizabeth Mastrantonio',
  'Julianne Nicholson', 'Saffron Burrows',

  // Trial by Jury
  'Bebe Neuwirth', 'Amy Carlson', 'Kirk Acevedo',

  // LA
  'Skeet Ulrich', 'Corey Stoll', 'Alfred Molina',
  'Terrence Howard', 'Rachel Ticotin', 'Megan Boone',

  // Organized Crime
  'Danielle Moné Truitt', 'Dylan McDermott', 'Brent Antonello',
  'Nona Parker Johnson', 'Rick Otto', 'Zuleikha Robinson',
  'Mykelti Williamson',

  // For the Defense
  'Margarita Levieva', 'Michael Imperioli', 'Odelya Halevi',
  'Mehcad Brooks',
]
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'drama; crime; television',
        casting_profile,
        notes: 'seeded via seed-law-order-actors.ts',
      }, { onConflict: 'tmdb_id' })
