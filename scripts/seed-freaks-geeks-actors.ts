/**
 * Seeds Freaks and Geeks cast members.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-freaks-geeks-actors.ts
 */
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'comedy; drama; television',
        casting_profile,
        notes: 'seeded via seed-freaks-geeks-actors.ts',
      }, { onConflict: 'tmdb_id' })
