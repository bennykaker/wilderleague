/**
 * Seeds missing short actors: Macaulay Culkin, Woody Allen, Bruno Mars, Salma Hayek.
 * Run with: npx tsx scripts/seed-short-missing.ts
 */
  ...
  for (const name of ACTORS) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        keywords: 'film; short',
        casting_profile,
        notes: 'seeded via seed-short-missing.ts',
      }, { onConflict: 'tmdb_id' })
