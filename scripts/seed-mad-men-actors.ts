/**
 * Seeds Mad Men cast members — series regulars and recurring players.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-mad-men-actors.ts
 */
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        known_for,
        keywords: 'drama; television; mad men',
        casting_profile,
        notes: 'seeded via seed-mad-men-actors.ts',
      }, { onConflict: 'tmdb_id' })
