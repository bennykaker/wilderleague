/**
 * Seeds NCIS franchise cast — original, LA, New Orleans, Hawai'i, Origins, Sydney.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-ncis-actors.ts
 */
...
const ACTORS = [
  // NCIS original
  'Mark Harmon', 'Michael Weatherly', 'Sasha Alexander', 'Pauley Perrette',
  'Sean Murray', 'David McCallum', 'Lauren Holly', 'Cote de Pablo',
  'Brian Dietzen', 'Rocky Carroll', 'Emily Wickersham', 'Wilmer Valderrama',
  'Maria Bello', 'Jennifer Esposito', 'Gary Cole', 'Katrina Law',
  'Diona Reasonover',

  // NCIS Los Angeles
  'Chris O\'Donnell', 'LL Cool J', 'Daniela Ruah', 'Eric Christian Olsen',
  'Barrett Foa', 'Renee Felice Smith', 'Miguel Ferrer', 'Medalion Rahimi',

  // NCIS New Orleans
  'Scott Bakula', 'Lucas Black', 'Zoe McLellan', 'Rob Kerkovich',
  'Daryl Mitchell', 'Necar Zadegan', 'Charles Michael Davis',
  'Vanessa Ferlito', 'Jason Alan Simmons',

  // NCIS Hawai'i
  'Vanessa Lachey', 'Alex Tarrant', 'Yasmine Al-Bustami', 'Jason Antoon',
  'Tori Anderson', 'Noah Mills', 'Seana Kofoed', 'Kian Talan',
  'Enver Gjokaj',

  // NCIS Origins
  'Austin Stowell', 'Kyle Schmid', 'Mariel Molino', 'Diany Rodriguez',
  'Robert Taylor', 'Caleb Foote', 'Tyla Abercrumbie',

  // NCIS Sydney
  'Todd Lasance', 'Olivia Swann', 'Sean Sagar', 'William McInnes',
  'Mavournee Hazel', 'Tuuli Narkle',
]
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        casting_profile,
        notes: 'seeded via seed-ncis-actors.ts',
      }, { onConflict: 'tmdb_id' })
