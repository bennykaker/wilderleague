/**
 * Seeds CSI franchise cast — original, Miami, NY, Cyber, Vegas Revival.
 * Skips anyone already in DB. Safe to re-run.
 * Run with: npx tsx scripts/seed-csi-actors.ts
 */
...
const ACTORS = [
  // CSI original
  'William Petersen', 'Marg Helgenberger', 'Gary Dourdan', 'George Eads',
  'Jorja Fox', 'Paul Guilfoyle', 'Robert David Hall', 'Eric Szmanda',
  'Wallace Langham', 'David Berman', 'Laurence Fishburne',
  'Katee Sackhoff', 'Ted Danson', 'Elisabeth Shue', 'Marc Vann',

  // CSI Miami
  'David Caruso', 'Emily Procter', 'Adam Rodriguez', 'Khandi Alexander',
  'Rex Linn', 'Jonathan Togo', 'Eva La Rue', 'Omar Miller',
  'Christian Clemenson', 'Eddie Cibrian',

  // CSI New York
  'Gary Sinise', 'Melina Kanakaredes', 'Carmine Giovinazzo',
  'Anna Belknap', 'Hill Harper', 'Eddie Cahill',
  'Robert Joy', 'AJ Buckley', 'Sela Ward',

  // CSI Cyber
  'Patricia Arquette', 'Peter MacNicol', 'Charley Koontz',
  'James Van Der Beek', 'Hayley Kiyoko',

  // CSI Vegas Revival
  'Paula Newsome', 'Matt Lauria', 'Mandeep Dhillon',
  'Mel Rodriguez', 'Jay Lee',
]
  ...
  for (const name of unique) {
    try {
      ...
      const { error } = await supabase.from('actors').upsert({
        ...
        casting_profile,
        notes: 'seeded via seed-csi-actors.ts',
      }, { onConflict: 'tmdb_id' })
